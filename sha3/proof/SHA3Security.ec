...
section Collision.
  ...
  + inline*; sp; wp. 
    ...
    - smt(spec_dout supp_dlist to_listK spec2_dout size_out_gt0). search dout.
  ...
  cut->:Pr[Indiff0.Indif(CSetSize(Sponge, Perm), Perm, D(A)).main() @ &m : res] =
        Pr[RealIndif(Sponge, Perm, DRestr(DSetSize(D(A)))).main() @ &m : res].
  + byequiv=>//=; proc. 
    inline Perm.init CSetSize(Sponge, Perm).init Sponge(Perm).init 
          FC(Sponge(Perm)).init; sp.
    inline D(A, CSetSize(Sponge, Perm), Perm).distinguish.
    inline DRestr(DSetSize(D(A)), Sponge(Perm), Perm).distinguish 
          DSetSize(D(A), FC(Sponge(Perm)), PC(Perm)).distinguish Cntr.init; wp; sp; sim. 
  ...
  cut->:Pr[Indiff0.Indif(RO, SimSetSize(Simulator, RO), D(A)).main() @ &m : res] =
        Pr[IdealIndif(BIRO.IRO, Simulator, DRestr(DSetSize(D(A)))).main() @ &m : res].
  + byequiv=>//=; proc.
    inline Simulator(FGetSize(RO)).init RO.init Simulator(BIRO.IRO).init 
          BIRO.IRO.init Gconcl_list.BIRO2.IRO.init; sp.
    inline D(A, RO, Simulator(FGetSize(RO))).distinguish.
    inline DRestr(DSetSize(D(A)), BIRO.IRO, Simulator(BIRO.IRO)).distinguish 
          DSetSize(D(A), FC(BIRO.IRO), PC(Simulator(BIRO.IRO))).distinguish; wp; sim.
    ...
    smt(mem_empty). print SHA3Indiff.
  have->//=:= SHA3Indiff (DSetSize(D(A))) &m _.
  move=> F P P_f_ll P_fi_ll F_ll; proc; inline*; auto; sp.
  seq 1 : true; auto. print A_ll.
